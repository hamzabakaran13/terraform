resource "github_repository" "new_repo" {
  for_each    = var.repos
  name        = "new-repo-${each.key}-${var.env}"
  ...
  provisioner "local-exec" {
    command = var.run_provisoners ? "gh repo view ${self.name} --web" : "echo 'Skipping view '"
  }
...
resource "terraform_data" "repo-clone" {
  ...
  provisioner "local-exec" {
    command = var.run_provisoners ? "gh repo clone ${github_repository.new_repo[each.key].name}" : "echo 'Skipping clone '"
  }
